...
(( ${+zbell_ignore} )) || zbell_ignore=($EDITOR $PAGER tig man)
...
zbell_end() {
	...
	if (( ! $has_ignored_cmd )) && (( ran_long )); then
		print -n "\a"
                #[[ $? -ne 0 ]] && 
                notify-send "Done" "Command \"$zbell_lastcmd\" has finished"
                # Avoid notifications if user just presses enter again
		zbell_timestamp=4294967295
	fi
}
